...
const USER_AGENT = "symphony-ts/0.2";
...
export class GitHubTracker {
  ...
  constructor(cfg: TrackerConfig) {
    // Construction never throws: a missing/invalid tracker config is a
    // dispatch-preflight concern (spec §6.3 / §14.2). It is surfaced as a
    // typed error only when an actual API call is attempted, so a missing
    // GITHUB_TOKEN keeps the service (and dashboard) alive instead of
    // crashing it on startup.
  ...
  /** Throw a typed error if the tracker is not usable for a live API call. */
  private ensureUsable(): void {
    if (this.ctx.cfg.kind !== "github") {
      throw new TrackerError(
        "unsupported_tracker_kind",
        `Unsupported tracker kind: ${this.ctx.cfg.kind}`,
      );
    }
    if (!this.ctx.token) {
      throw new TrackerError(
        "missing_tracker_api_key",
        "tracker.api_key is required for this profile",
      );
    }
  }
